...
const WEBSETTINGS = path.join(__dirname, "./db/webSettings.json");
...
app.post("/add-banners",upload.array('banners', 10), (req, res) => {
    ...
    fs.readFile(WEBSETTINGS, "utf8", (err, data) => {
        let jsonData = [];
        if (!err && data) {
            jsonData = JSON.parse(data);
        }
        let bannerLength = jsonData.banners.length 
        let objectData = []; 
        ...
        jsonData = {...jsonData, banners : [...jsonData.banners, ...objectData]}
        fs.writeFile(WEBSETTINGS, JSON.stringify(jsonData, null, 2), (err) => {
            if (err) {
                return res.status(500).json({ message: "Error saving data" });
            }
...
app.post("/delete-banner",upload.array('banners', 10), (req, res) => {
    const filePath = req.body.imgId;
    ...
    fs.readFile(WEBSETTINGS, "utf8", (err, data) => {
        ...
        newBanners = jsonData.banners.filter(x=>x.id != filePath)
            jsonData = {...jsonData, banners : newBanners}
        fs.writeFile(WEBSETTINGS, JSON.stringify(jsonData, null, 2), (err) => {
            if (err) {
                return res.status(500).json({ message: "Error saving data" });
            }
